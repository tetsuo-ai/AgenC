import { TOOL_DEFINITIONS, } from "@tetsuo-ai/desktop-tool-contracts";
export { TOOL_DEFINITIONS } from "@tetsuo-ai/desktop-tool-contracts";
// Re-export public APIs from sub-modules
export { subscribeDesktopToolEvents, __managedProcessTestHooks, } from "./tools-process.js";
// --- Input tools ---
import { mouseClick, mouseMove, mouseDrag, mouseScroll, keyboardType, keyboardKey, } from "./tools-input.js";
// --- Window management tools ---
import { windowList, windowFocus, clipboardGet, clipboardSet, screenSize, } from "./tools-window.js";
// --- Process & bash tools ---
import { bash, processStart, processStatus, processStop, } from "./tools-process.js";
// --- Media tools ---
...
export function validateDesktopToolHandlers(definitions = TOOL_DEFINITIONS) {
    const missingHandlers = definitions
        .map((definition) => definition.name)
        .filter((name) => !(name in handlers));
    if (missingHandlers.length > 0) {
        throw new Error(`Desktop tool contract is missing handlers for: ${missingHandlers.join(", ")}`);
    }
    ...
    if (orphanHandlers.length > 0) {
        throw new Error(`Desktop tool handlers are missing contract definitions for: ${orphanHandlers.join(", ")}`);
    }
}
