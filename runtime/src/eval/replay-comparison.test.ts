...
import { computeProjectionHash, type ReplayTimelineRecord } from '../replay/types.js';
import {
...
function extractDisputeIdFromRecord(record: ReplayTimelineRecord): string | undefined {
  const payload = record.payload as { disputeId?: string; onchain?: { disputeId?: string } };
...
describe('ReplayComparisonService', () => {
  it('produces a clean report for matching projected and local traces', async () => {
    ...
    expect(report.localReplay.deterministicHash).toBe(report.projectedReplay.deterministicHash);
  });
  ...
  it('throws in strict mode and exposes machine-readable report', async () => {
    ...
    const perturbed = records.map((record, index) => index === 1
      ? { ...record, taskPda: 'bad-task', projectionHash: record.projectionHash }
      : record);

    await expect(new ReplayComparisonService().compare({
      projected: perturbed,
      localTrace: projection.trace,
      options: {
        strictness: 'strict',
  ...
  it('includes dispute IDs in diagnostics when matching on dispute timelines', async () => {
    ...
    const localWithoutDispute = {
      ...projection.trace,
      events: projection.trace.events.filter((entry) => entry.type !== 'dispute:initiated'),
    };
    ...
    expect(report.status).toBe('mismatched');
    const missing = report.anomalies.find((entry) => entry.code === 'missing_event');
  ...
  it('emits comparison metrics with mismatch and latency labels', async () => {
    const onChain = taskCreatedEvents();
    const projection = projectOnChainEvents(onChain, { traceId: 'cmp-metrics' });
    const records = makeRecordsWithHash(projection.events);
    const perturbed = records.map((entry, index) => index === 0
      ? { ...entry, type: 'completed' as const }
      : entry);

    const metrics: { calls: Array<{ name: string; value?: number; labels?: Record<string, string> }> } = {
      calls: [],
    };
    ...
    expect(counterNames).toContain('agenc.replay.comparison.anomaly.hash_mismatch');
    expect(counterNames).toContain('agenc.replay.comparison.anomaly.event_mismatch');
    expect(counterNames).toContain('agenc.replay.comparison.resolution_latency_ms');
  });
});
