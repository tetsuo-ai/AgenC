...
function makeWorkflow(): WorkflowDefinition {
  return {
    ...
    edges: [
      { from: 'root', to: 'child-a', dependencyType: OnChainDependencyType.Data },
      { from: 'root', to: 'child-b', dependencyType: OnChainDependencyType.Ordering },
    ],
  };
}
...
function candidateSignature(candidate: WorkflowMutationCandidate): string {
  const tasks = [...candidate.definition.tasks]
    ...
    .map((task) => `${task.name}:${task.taskType}:${task.rewardAmount}:${task.deadline}`)
    .join('|');
...
describe('generateWorkflowMutationCandidates', () => {
  it('produces deterministic candidates for a fixed seed', () => {
    ...
    const first = generateWorkflowMutationCandidates(baseline, { seed: 42, maxCandidates: 6 });
    const second = generateWorkflowMutationCandidates(baseline, { seed: 42, maxCandidates: 6 });

    expect(first.map((candidate) => candidate.id)).toEqual(second.map((candidate) => candidate.id));
    expect(first.map(candidateSignature)).toEqual(second.map(candidateSignature));
  });

  it('ensures all generated variants pass workflow validation', () => {
    const baseline = makeWorkflow();
    const candidates = generateWorkflowMutationCandidates(baseline, { seed: 9, maxCandidates: 10 });
  ...
  it('covers the expected mutation operator families and enforces max candidate limit', () => {
    const baseline = makeWorkflow();
    const candidates = generateWorkflowMutationCandidates(baseline, { seed: 7, maxCandidates: 5 });

    expect(candidates.length).toBeLessThanOrEqual(5);

    const operators = new Set(
      candidates.flatMap((candidate) => candidate.mutations.map((mutation) => mutation.operator)),
    );
