...
import type { DailyLogManager, CuratedMemoryManager, EntityExtractor, StructuredMemoryEntry } from './structured.js';
...
function createMockEmbeddingProvider(): EmbeddingProvider {
  return {
    name: 'test',
    dimension: 128,
    embed: vi.fn<[string], Promise<number[]>>().mockResolvedValue(new Array(128).fill(0)),
...
function createMockVectorStore(): VectorMemoryBackend {
  ...
  return {
    ...
    getDurability: vi.fn().mockReturnValue({ level: 'none', supportsFlush: false, description: 'mock' }),
...
function createMockCuratedMemory(): CuratedMemoryManager {
  return {
    proposeAddition: vi.fn((fact: string, source: string) => `- ${fact} (source: ${source})`),
...
function createMockEntityExtractor(): EntityExtractor {
  return {
    extract: vi.fn<[string, string], Promise<StructuredMemoryEntry[]>>().mockResolvedValue([]),
  };
}
...
function createMockLLMProvider(): LLMProvider {
  ...
  return {
    name: 'test-llm',
    chat: vi.fn<[LLMMessage[]], Promise<LLMResponse>>().mockResolvedValue(response),
...
describe('MemoryIngestionEngine', () => {
  ...
  function createEngine(overrides?: Partial<IngestionConfig>): MemoryIngestionEngine {
  ...
  describe('ingestTurn', () => {
    ...
    it('appends user + agent messages to daily log', async () => {
      ...
      expect(logManager.append).toHaveBeenCalledWith('sess-1', 'assistant', 'hi there');
    });
    ...
    it('handles vector store failure gracefully (still appends daily log)', async () => {
      (vectorStore.storeWithEmbedding as ReturnType<typeof vi.fn>).mockRejectedValue(
    ...
    it('handles daily log failure gracefully (does not throw)', async () => {
      ...
      await expect(engine.ingestTurn('sess-1', 'hello', 'hi there')).resolves.toBeUndefined();
  ...
  describe('processSessionEnd', () => {
    ...
    it('generates summary via LLM with correct message format', async () => {
      ...
      expect(llmProvider.chat).toHaveBeenCalledWith([
        {
          role: 'system',
          content: 'Summarize this conversation in 2-3 sentences, focusing on key decisions and learnings.',
        },
        {
          role: 'user',
    ...
    it('extracts entities and formats as proposed facts', async () => {
      ...
      expect(result.proposedFacts).toEqual(['- Solana is fast (source: conversation)']);
      expect(curatedMemory.proposeAddition).toHaveBeenCalledWith('Solana is fast', 'conversation');
    });
  ...
  describe('processCompaction', () => {
    ...
    it('passes correct sessionId to storeWithEmbedding', async () => {
      ...
      const call = (vectorStore.storeWithEmbedding as ReturnType<typeof vi.fn>).mock.calls[0];
    ...
    it('handles embedding failure gracefully', async () => {
      ...
      await expect(engine.processCompaction('sess-1', 'Some summary')).resolves.toBeUndefined();
      expect(logger.error).toHaveBeenCalledWith(
  ...
  describe('createIngestionHooks', () => {
    ...
    it('session:compact handler awaits processCompaction', async () => {
      const engine = createEngine();
      const compactSpy = vi.spyOn(engine, 'processCompaction').mockResolvedValue(undefined);
      const hooks = createIngestionHooks(engine, logger);
    ...
    it('all hooks return { continue: true } even on error', async () => {
      const engine = createEngine();
      vi.spyOn(engine, 'processSessionEnd').mockRejectedValue(new Error('boom'));
      vi.spyOn(engine, 'processCompaction').mockRejectedValue(new Error('boom'));
      const hooks = createIngestionHooks(engine, logger);
