...
describe('PID file operations', () => {
  ...
  it('writePidFile writes JSON with pid, port, configPath', async () => {
    ...
    const info: PidFileInfo = { pid: 12345, port: 8080, configPath: '/tmp/config.json' };
  ...
  it('readPidFile parses JSON correctly', async () => {
    const pidPath = join(tempDir, 'test.pid');
    const info: PidFileInfo = { pid: 42, port: 9090, configPath: '/etc/agenc.json' };
    await writePidFile(info, pidPath);
...
describe('checkStalePid', () => {
  ...
  it('returns alive when process is running', async () => {
    ...
    await writePidFile({ pid: process.pid, port: 8080, configPath: '/c' }, pidPath);
  ...
  it('returns stale when process is not running', async () => {
    ...
    await writePidFile({ pid: 99999999, port: 8080, configPath: '/c' }, pidPath);
...
describe('DaemonManager', () => {
  ...
  it('setupSignalHandlers is idempotent', () => {
    ...
    const signalCalls = onSpy.mock.calls.filter(
      (call) => ['SIGTERM', 'SIGINT', 'SIGHUP'].includes(call[0] as string),
    );
