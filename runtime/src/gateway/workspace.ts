...
function validateWorkspaceConfig(raw: unknown): string[] {
  ...
  if (raw.skills !== undefined) {
    if (!Array.isArray(raw.skills) || !raw.skills.every((s: unknown) => typeof s === 'string')) {
  ...
  if (raw.capabilities !== undefined) {
    if (typeof raw.capabilities !== 'string') {
      ...
      try {
        ...
        errors.push(`capabilities is not a valid decimal bigint string: "${raw.capabilities}"`);
      }
    }
  }
  ...
  if (raw.toolPermissions !== undefined) {
    if (!Array.isArray(raw.toolPermissions)) {
      ...
      for (let i = 0; i < raw.toolPermissions.length; i++) {
        ...
        if (!isRecord(tp) || typeof tp.tool !== 'string' || typeof tp.allow !== 'boolean') {
          errors.push(`toolPermissions[${i}] must have {tool: string, allow: boolean}`);
        }
      }
    }
  }
...
async function loadWorkspaceConfig(workspacePath: string): Promise<WorkspaceConfigJson> {
  ...
  try {
    ...
    if (err instanceof Error && 'code' in err && (err as NodeJS.ErrnoException).code === 'ENOENT') {
      throw new WorkspaceValidationError('config', `not found at ${configPath}`);
    }
    throw err;
  }
  ...
  try {
    ...
    throw new WorkspaceValidationError('config', `Invalid JSON in ${configPath}`);
  }
...
function resolveAgentWorkspace(
  ...
  return {
    ...
    llm: config.llm,
    memoryNamespace: config.memoryNamespace ?? `${MEMORY_NAMESPACE_PREFIX}${id}:`,
    capabilities,
...
export class WorkspaceManager {
  ...
  async createWorkspace(
    ...
    if (template?.capabilities !== undefined) {
      config.capabilities = parseCapabilities([...template.capabilities]).toString();
    }
