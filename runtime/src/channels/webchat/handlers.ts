...
function parseRawTaskAccount(data: Buffer): {
  ...
  const creator = new PublicKey(
    data.subarray(TASK_CREATOR_OFFSET, TASK_CREATOR_OFFSET + 32),
  );
  ...
  const descBuf = data.subarray(
    TASK_DESCRIPTION_OFFSET,
    TASK_DESCRIPTION_OFFSET + 64,
  );
  const nullIdx = descBuf.indexOf(0);
  const description = new TextDecoder().decode(
    descBuf.subarray(0, nullIdx === -1 ? 64 : nullIdx),
  );
  return {
    status: statusByte as OnChainTaskStatus,
    reward,
    creator,
    currentWorkers,
    description,
...
async function sendTaskList(
  ...
  const makeFilter = (statusByte: number) => [
    { memcmp: { offset: 0, bytes: bs58.default.encode(TASK_DISCRIMINATOR) } },
    {
      memcmp: {
        ...
        bytes: bs58.default.encode(Buffer.from([statusByte])),
      },
    },
  ];
  ...
  const [openAccounts, inProgressAccounts] = await Promise.all([
    connection.getProgramAccounts(programId, {
      filters: makeFilter(OnChainTaskStatus.Open),
    }),
    connection.getProgramAccounts(programId, {
      filters: makeFilter(OnChainTaskStatus.InProgress),
    }),
  ]);
...
export async function handleTasksCreate(
  ...
  const keypairPath =
    deps.gateway.config.connection?.keypairPath ?? getDefaultKeypairPath();
  try {
    ...
    const descStr =
      typeof (params as Record<string, unknown>).description === "string"
        ...
        : "Task from WebUI";
    const rewardInput =
      typeof (params as Record<string, unknown>).reward === "number"
    ...
    const rewardLamports = BigInt(
      Math.max(Math.round(rewardInput * 1_000_000_000), 10_000_000),
    );
    ...
    const agentAccounts = await deps.connection.getProgramAccounts(
      program.programId,
      {
        filters: [
          {
            memcmp: {
              offset: 0,
              bytes: bs58Mod.default.encode(AGENT_DISCRIMINATOR),
            },
          },
          {
            memcmp: {
    ...
    if (agentAccounts.length === 0) {
      send({
        type: "error",
        error:
          "No agent registration found for this wallet. Register an agent first.",
        id,
      });
      return;
    }
...
export async function handleTasksCancel(
  ...
  try {
    ...
    send({
      type: "error",
      error: `Failed to cancel task: ${(err as Error).message}`,
      id,
    });
  }
}
...
export async function handleMemorySearch(
  ...
  try {
    ...
    send({
      type: "error",
      error: `Memory search failed: ${(err as Error).message}`,
      id,
    });
  }
}
  ...
  try {
    ...
    for (const sid of sessions.slice(0, limit)) {
      ...
      results.push({
        ...
        lastActiveAt:
          thread.length > 0 ? thread[thread.length - 1].timestamp : 0,
      });
    }
    ...
    send({
      type: "error",
      error: `Memory sessions failed: ${(err as Error).message}`,
      id,
    });
  }
}
