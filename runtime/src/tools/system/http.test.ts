...
describe('isDomainAllowed', () => {
  ...
  describe('SSRF protection', () => {
    ...
    it('blocks 10.0.0.0/8 (private class A)', () => {
      ...
      expect(isDomainAllowed('http://10.255.255.255/admin').allowed).toBe(false);
    });

    it('blocks 172.16.0.0/12 (private class B)', () => {
      expect(isDomainAllowed('http://172.16.0.1/admin').allowed).toBe(false);
      expect(isDomainAllowed('http://172.31.255.255/admin').allowed).toBe(false);
    ...
    it('blocks *.internal wildcard', () => {
      expect(isDomainAllowed('http://service.internal:8080').allowed).toBe(false);
    });

    it('blocks IPv4-mapped IPv6 private addresses', () => {
      expect(isDomainAllowed('http://[::ffff:127.0.0.1]:8080').allowed).toBe(false);
      expect(isDomainAllowed('http://[::ffff:10.0.0.1]:8080').allowed).toBe(false);
      expect(isDomainAllowed('http://[::ffff:192.168.1.1]:8080').allowed).toBe(false);
    });
  });
});
...
describe('redirects', () => {
  ...
  it('stops after maxRedirects', async () => {
    ...
    const [httpGet] = createHttpTools({}, silentLogger);
    const result = await httpGet.execute({ url: 'https://example.com/redirect-0' });
...
describe('error handling', () => {
  ...
  it('SSRF blocked via tool execute returns isError: true', async () => {
    const [httpGet] = createHttpTools({}, silentLogger);
    const result = await httpGet.execute({ url: 'http://localhost:8899/health' });
