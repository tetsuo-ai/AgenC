...
metadata:
  agenc:
    ...
    install:
      - type: brew
        package: noir-lang/noir/risc0-host-prover
      - type: download
...
Call \`risc0-host-prover prove\` with the circuit path.
`;
...
describe('parseSkillContent', () => {
  it('parses valid SKILL.md with all fields', () => {
    ...
    // Requirements
    expect(skill.metadata.requires.binaries).toEqual(['risc0-host-prover', 'bb']);
  ...
  it('body preserves markdown formatting', () => {
    ...
    expect(skill.body).toContain('`risc0-host-prover prove`');
  });
...
`;
    ...
    expect(skill.metadata.install[1]).toEqual({ type: 'cargo', package: 'risc0-host-prover' });
