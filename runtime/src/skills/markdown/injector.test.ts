...
function makeDiscovered(skill: MarkdownSkill, tier: 'agent' | 'user' | 'project' | 'builtin' = 'builtin'): DiscoveredSkill {
...
function makeMockDiscovery(skills: DiscoveredSkill[]): SkillDiscovery {
  ...
  discovery.getAvailable = vi.fn<() => Promise<DiscoveredSkill[]>>().mockResolvedValue(skills);
  return discovery;
}
...
describe('MarkdownSkillInjector', () => {
  ...
  describe('inject', () => {
    it('returns formatted <skill> blocks for matching skills', async () => {
      ...
      const injector = makeInjector([makeDiscovered(skill)], { logger: silentLogger });
      const result = await injector.inject('open a github repository', 'session-1');
    ...
    it('returns undefined when no skills match', async () => {
      ...
      const injector = makeInjector([makeDiscovered(skill)], { logger: silentLogger });
      const result = await injector.inject('solana blockchain defi', 'session-1');
    ...
    it('filters by capability bitmask', async () => {
      ...
      const result1 = await injector1.inject('/skill inference-skill', 'session-1');
      ...
      const result2 = await injector2.inject('/skill inference-skill', 'session-1');
    ...
    it('respects token budget', async () => {
      ...
      const result = await injector.injectDetailed('/skill big /skill small', 'session-1');
    ...
    it('prioritizes higher-scoring skills', async () => {
      ...
      const result = await injector.injectDetailed('create github pull request', 'session-1');
  ...
  describe('injectDetailed', () => {
    it('returns injected/excluded lists and token count', async () => {
      ...
      const result = await injector.injectDetailed('/skill skill-a /skill skill-b', 'session-1');
