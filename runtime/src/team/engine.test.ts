...
import { TeamContractStateError, TeamContractValidationError } from './errors.js';
...
describe('TeamContractEngine', () => {
  ...
  it('freezes roster mutations after start', () => {
    const engine = new TeamContractEngine();
    engine.createContract({ contractId: 'c2', creatorId: 'creator', template: makeTemplate() });
    ...
    expect(() => engine.leaveContract('c2', 'p1')).toThrow('cannot leave members');
    expect(() =>
      engine.assignRole({ contractId: 'c2', memberId: 'w1', roleId: 'planner' }),
    ).toThrow('cannot assign roles');
  });

  it('gates downstream checkpoints by dependency completion', () => {
    const engine = new TeamContractEngine();
    engine.createContract({ contractId: 'c3', creatorId: 'creator', template: makeTemplate() });
  ...
  it('propagates failure and isolates re-entrant hooks', () => {
    ...
    engine.createContract({ contractId: 'c4', creatorId: 'creator', template: makeTemplate() });
  ...
  it('finalize is idempotent and immutable', () => {
    const engine = new TeamContractEngine();
    engine.createContract({ contractId: 'c5', creatorId: 'creator', template: makeTemplate() });
    ...
    engine.startRun('c5');
    engine.completeCheckpoint({ contractId: 'c5', checkpointId: 'plan', memberId: 'p1' });
    engine.completeCheckpoint({ contractId: 'c5', checkpointId: 'build', memberId: 'w1' });
    engine.completeCheckpoint({ contractId: 'c5', checkpointId: 'review', memberId: 'r1' });

    const first = engine.finalizePayout({ contractId: 'c5', totalRewardLamports: 1_000n });
    const second = engine.finalizePayout({ contractId: 'c5', totalRewardLamports: 1_000n });
  ...
  it('supports planner-worker-reviewer end-to-end lifecycle', () => {
    const engine = new TeamContractEngine();
    engine.createContract({ contractId: 'c8', creatorId: 'creator', template: makeTemplate() });
    ...
    engine.completeCheckpoint({ contractId: 'c8', checkpointId: 'plan', memberId: 'planner-1' });
    engine.completeCheckpoint({ contractId: 'c8', checkpointId: 'build', memberId: 'worker-1' });
    engine.completeCheckpoint({ contractId: 'c8', checkpointId: 'review', memberId: 'reviewer-1' });

    const payout = engine.finalizePayout({ contractId: 'c8', totalRewardLamports: 1_000n });
