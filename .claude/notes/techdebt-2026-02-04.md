...
|-------|--------|-------------|
| Placeholder Poseidon hash | **FIXED** | Now uses `poseidon-lite` via `computeConstraintHashFromProofs` and `computeCommitmentFromProofs` |
...
|------|------|------|----------|
| `sdk/src/privacy.ts` | 363-365 | `// TODO: Use actual Poseidon hash implementation` + security warning | CRITICAL |
...
- **Recommended priority:** Fix placeholder Poseidon hash (security-critical)

## Recommended Refactoring Order

1. **Immediate (Day 1):** Fix `validateCircuitPath()` duplication, fix CAPABILITY_STORAGE bug
2. **This week:** Implement real Poseidon hash, resolve proof size discrepancy
