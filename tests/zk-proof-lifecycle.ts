...
describe("ZK Proof Verification Lifecycle (router payload)", () => {
  ...
  const creatorAgentId = Buffer.from(
    `zk-creator-${runId}`.slice(0, 32).padEnd(32, "\0"),
  );
  const workerAgentId = Buffer.from(
    `zk-worker-${runId}`.slice(0, 32).padEnd(32, "\0"),
  );
  ...
  function createProofPayload(params: {
    ...
    const outputCommitment =
      params.outputCommitment ?? Buffer.alloc(HASH_SIZE, 0x51);
  ...
  it("submits complete_task_private with dual-spend + router accounts", async () => {
    ...
    const { taskId, taskPda, escrowPda, claimPda } =
      await createPrivateTaskAndClaim(constraintHash);
