...
} from "./lib/agenc-watch-surface-summary.mjs";

function normalizeSnapshot(lines) {
  return lines.map((line) => String(line ?? "").replace(/\s+$/g, "")).join("\n");
}
...
test("frame snapshot keeps wide planner and sidebar chrome stable", () => {
  ...
  const snapshot = normalizeSnapshot(harness.controller.buildVisibleFrameSnapshot().lines);
  assert.equal(snapshot, FRAME_SNAPSHOT_EXPECTATIONS.widePlanner);
});
...
test("frame snapshot keeps diff detail navigation stable", () => {
  ...
  const snapshot = normalizeSnapshot(harness.controller.buildVisibleFrameSnapshot().lines);
  assert.equal(snapshot, FRAME_SNAPSHOT_EXPECTATIONS.diffDetail);
});
...
test("frame snapshot keeps narrow reconnect view stable", () => {
  ...
  const snapshot = normalizeSnapshot(harness.controller.buildVisibleFrameSnapshot().lines);
  assert.equal(snapshot, FRAME_SNAPSHOT_EXPECTATIONS.narrowReconnect);
});
