...
function truncateAnsi(text, maxChars, color) {
...
export function createWatchFrameController(dependencies = {}) {
  ...
  function renderCompactSplash(width, height) {
    ...
    const tone = watchState.bootstrapReady && transportState.connectionState === "live" ? "teal" : "magenta";
    ...
    const content = [
      ...
      centerAnsi(`${color.fog}${truncate(sanitizeInlineText(hint), Math.max(24, width - 6))}${color.reset}`, width, fitAnsi),
    ];
  ...
  function renderSplash(width, height) {
    ...
    const content = [
      centerAnsi(`${color.magenta}${color.bold}A G E N / C${color.reset} ${color.softInk}https://agenc.tech${color.reset}`, width, fitAnsi),
      centerAnsi(`${color.fog}clean signal // low clutter // live autonomy${color.reset}`, width, fitAnsi),
      "",
