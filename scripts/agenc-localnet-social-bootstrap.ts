...
const DEFAULT_OPERATOR_KEYPAIR = path.join(
  os.homedir(),
  ".config",
  "solana",
  "id.json",
...
const DEFAULT_BASE_CONFIG = path.join(os.homedir(), ".agenc", "config.json");
const DEFAULT_GATEWAY_BASE_PORT = 3101;
const DEFAULT_MESSAGING_BASE_PORT = 4101;
...
function usage(): void {
  ...
  npx tsx scripts/agenc-localnet-social-bootstrap.ts [options]
...
function parseArgs(argv: string[]): CliOptions {
  ...
  if (
    !Number.isInteger(options.messagingBasePort) ||
    options.messagingBasePort < 1
  ) {
