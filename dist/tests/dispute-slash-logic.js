...
describe("dispute-slash-logic (issue #136)", () => {
    ...
    const WORKER_STAKE = 10 * web3_js_1.LAMPORTS_PER_SOL;
    ...
    const ensureProtocol = async () => {
        try {
            ...
            const minStake = new bn_js_1.default(web3_js_1.LAMPORTS_PER_SOL); // 1 SOL
            const minStakeForDispute = new bn_js_1.default(web3_js_1.LAMPORTS_PER_SOL / 10); // 0.1 SOL
    ...
    before(async () => {
        ...
        thirdSigner = web3_js_1.Keypair.generate(); // Required for multisig threshold >= 2
        ...
        await registerAgent(workerAgentId, worker, test_utils_1.CAPABILITY_COMPUTE, actualWorkerStake);
    ...
    describe("applyDisputeSlash preconditions", () => {
        it("should fail if dispute is not resolved (DisputeNotResolved error)", async () => {
            ...
            // 1. Create task
            await program.methods
                .createTask(Array.from(taskId), new bn_js_1.default(test_utils_1.CAPABILITY_COMPUTE), Buffer.from("Test task for precondition".padEnd(64, "\0")), new bn_js_1.default(web3_js_1.LAMPORTS_PER_SOL), 1, (0, test_utils_1.getDefaultDeadline)(), test_utils_1.TASK_TYPE_EXCLUSIVE, null, 0, // min_reputation
            null)
                .accountsPartial({
                task: taskPda,
                escrow: escrowPda,
            ...
            await program.methods
                .initiateDispute(Array.from(disputeId), Array.from(taskId), Array.from(Buffer.from("evidence-hash".padEnd(32, "\0"))), test_utils_1.RESOLUTION_TYPE_REFUND, VALID_EVIDENCE)
                .accountsPartial({
                dispute: disputePda,
                task: taskPda,
            ...
            try {
                await program.methods
                    ...
                    .rpc();
                chai_1.expect.fail("Should have failed - dispute not resolved");
            }
            catch (e) {
        ...
        it("should verify dispute can be voted on by arbiters", async () => {
            ...
            // 1. Create task
            await program.methods
                .createTask(Array.from(taskId), new bn_js_1.default(test_utils_1.CAPABILITY_COMPUTE), Buffer.from("Test task for voting".padEnd(64, "\0")), new bn_js_1.default(web3_js_1.LAMPORTS_PER_SOL), 1, (0, test_utils_1.getDefaultDeadline)(), test_utils_1.TASK_TYPE_EXCLUSIVE, null, 0, // min_reputation
            null)
                .accountsPartial({
                task: taskPda,
                escrow: escrowPda,
            ...
            (0, chai_1.expect)(disputeAfterVote.votesAgainst.toNumber()).to.be.greaterThan(0);
            (0, chai_1.expect)(disputeAfterVote.votesFor.toNumber()).to.equal(0);
        });
    });
    describe("Issue #136 fix verification (code review)", () => {
        ...
        it("documents the fix for issue #136", async () => {
            ...
            const config = await program.account.protocolConfig.fetch(protocolPda);
            (0, chai_1.expect)(config.slashPercentage).to.be.greaterThan(0);
    ...
    describe("defendant binding (issue #827)", () => {
        ...
        let workerBAgentId;
        before(async () => {
            workerB = web3_js_1.Keypair.generate();
            workerBAgentId = makeId("wrB");
            await airdrop([workerB]);
            const actualWorkerStake = Math.max(WORKER_STAKE, minAgentStake);
            await registerAgent(workerBAgentId, workerB, test_utils_1.CAPABILITY_COMPUTE, actualWorkerStake);
        });
        it("should set defendant field and reject slash on wrong worker", async () => {
            ...
            await program.methods
                .createTask(Array.from(taskId), new bn_js_1.default(test_utils_1.CAPABILITY_COMPUTE), Buffer.from("Test task for defendant binding".padEnd(64, "\0")), new bn_js_1.default(web3_js_1.LAMPORTS_PER_SOL), 3, // max_workers
            ...
            const dispute = await program.account.dispute.fetch(disputePda);
            (0, chai_1.expect)(dispute.defendant.toBase58()).to.equal(workerAgentPda.toBase58());
            ...
            try {
                ...
                if (code !== undefined) {
                    (0, chai_1.expect)(["WorkerNotInDispute", "DisputeNotResolved"]).to.include(code);
                }
            }
            (0, chai_1.expect)(rejected).to.equal(true, "slash should be rejected for non-defendant worker");
        });
    });
    // =========================================================================
    ...
    async function setupDispute(prefix, resolutionType = test_utils_1.RESOLUTION_TYPE_REFUND, rewardLamports = web3_js_1.LAMPORTS_PER_SOL, wrkKp = worker, wrkAgentId = workerAgentId) {
        ...
        const disputePda = deriveDisputePda(disputeId);
        const deadline = new bn_js_1.default((0, litesvm_helpers_1.getClockTimestamp)(svm) + 2000000);
        await program.methods
            .createTask(Array.from(taskId), new bn_js_1.default(test_utils_1.CAPABILITY_COMPUTE), Buffer.from("Dispute test task".padEnd(64, "\0")), new bn_js_1.default(rewardLamports), 1, deadline, test_utils_1.TASK_TYPE_EXCLUSIVE, null, 0, null)
            .accountsPartial({
            task: taskPda,
            escrow: escrowPda,
            protocolConfig: protocolPda,
        ...
        await program.methods
            ...
            .rpc();
        await program.methods
            .initiateDispute(Array.from(disputeId), Array.from(taskId), Array.from(Buffer.from("evidence-hash".padEnd(32, "\0"))), resolutionType, VALID_EVIDENCE)
            .accountsPartial({
            dispute: disputePda,
            task: taskPda,
            agent: crePda,
    ...
    async function vote(disputePda, taskPda, claimPda, wrkPda, kp, agentId, approve) {
    ...
    async function voteAll(disputePda, taskPda, claimPda, wrkPda, votes) {
        const arbs = [
    ...
    function resolveRemaining(voters) {
    ...
    async function advanceAndResolve(disputePda, taskPda, escrowPda, claimPda, wrkPda, voters, wrkAuthority = worker) {
        const d = await program.account.dispute.fetch(disputePda);
        const dl = d.votingDeadline.toNumber();
        const now = (0, litesvm_helpers_1.getClockTimestamp)(svm);
        if (now < dl)
            (0, litesvm_helpers_1.advanceClock)(svm, dl - now);
        await program.methods
            .resolveDispute()
            .accountsPartial({
            dispute: disputePda,
    ...
    describe("Vote window boundary math (#960)", () => {
        it("accepts vote 1 second before voting_deadline", async () => {
            ...
            const d = await program.account.dispute.fetch(disputePda);
            (0, litesvm_helpers_1.advanceClock)(svm, d.votingDeadline.toNumber() - (0, litesvm_helpers_1.getClockTimestamp)(svm) - 1);
        ...
        it("rejects vote exactly at voting_deadline (VotingEnded)", async () => {
            ...
            const d = await program.account.dispute.fetch(disputePda);
            (0, litesvm_helpers_1.advanceClock)(svm, d.votingDeadline.toNumber() - (0, litesvm_helpers_1.getClockTimestamp)(svm));
            try {
                await vote(disputePda, taskPda, claimPda, workerPda, arbiter1, arbiter1AgentId, true);
                chai_1.expect.fail("Should have failed with VotingEnded");
            }
            catch (e) {
                (0, chai_1.expect)(e.error?.errorCode?.code).to.equal("VotingEnded");
            }
        });
        ...
        it("rejects resolve 1 second before voting_deadline (VotingNotEnded)", async () => {
            ...
            try {
                ...
                (0, chai_1.expect)(e.error?.errorCode?.code).to.equal("VotingNotEnded");
            }
        });
    });
    // =========================================================================
    ...
    function disputeSlashAccounts(s) {
        return {
            dispute: s.disputePda,
            task: s.taskPda,
            workerClaim: s.claimPda,
    ...
    function anchorErrorCode(e) {
        const err = e;
        return err.error?.errorCode?.code ?? err.errorCode?.code;
    }
    /** Assert an Anchor error code was thrown (falls back to message check). */
    function expectErrorCode(e, code) {
        const got = anchorErrorCode(e);
        if (got) {
            (0, chai_1.expect)(got).to.equal(code);
        }
        else {
            const msg = e.message ?? String(e);
            (0, chai_1.expect)(msg).to.include(code, `Expected error ${code}, got: ${msg.slice(0, 200)}`);
        }
    }
    // =========================================================================
    ...
    describe("Slash idempotency and window (#960)", () => {
        let slashWorker;
        let slashWorkerAgentId;
        before(async () => {
            slashWorker = web3_js_1.Keypair.generate();
            slashWorkerAgentId = makeId("swk");
            airdrop([slashWorker]);
            await registerAgent(slashWorkerAgentId, slashWorker, test_utils_1.CAPABILITY_COMPUTE, Math.max(WORKER_STAKE, minAgentStake));
        });
        it("applies worker slash exactly once (SlashAlreadyApplied on retry)", async () => {
            const s = await setupDispute("si1", test_utils_1.RESOLUTION_TYPE_REFUND, web3_js_1.LAMPORTS_PER_SOL, slashWorker, slashWorkerAgentId);
            const voters = await voteAll(s.disputePda, s.taskPda, s.claimPda, s.workerPda, [true, true, true]);
            ...
            await program.methods
                ...
                .rpc();
            const d = await program.account.dispute.fetch(s.disputePda);
            (0, chai_1.expect)(d.slashApplied).to.equal(true, "slash_applied should be set after first slash");
            ...
            (0, chai_1.expect)(rejected).to.equal(true, "Second applyDisputeSlash must be rejected");
        });
        it("applies initiator slash exactly once (SlashAlreadyApplied on retry)", async () => {
            const s = await setupDispute("si2", test_utils_1.RESOLUTION_TYPE_REFUND, web3_js_1.LAMPORTS_PER_SOL, slashWorker, slashWorkerAgentId);
            ...
            await program.methods
                ...
                .rpc();
            const d = await program.account.dispute.fetch(s.disputePda);
            (0, chai_1.expect)(d.initiatorSlashApplied).to.equal(true, "initiator_slash_applied should be set");
            ...
            (0, chai_1.expect)(rejected).to.equal(true, "Second applyInitiatorSlash must be rejected");
        });
        it("rejects slash after 7-day window (SlashWindowExpired)", async () => {
            const SLASH_WINDOW = 7 * 24 * 60 * 60; // 604800
            const s = await setupDispute("si3", test_utils_1.RESOLUTION_TYPE_REFUND, web3_js_1.LAMPORTS_PER_SOL, slashWorker, slashWorkerAgentId);
            const voters = await voteAll(s.disputePda, s.taskPda, s.claimPda, s.workerPda, [true, true, true]);
            await advanceAndResolve(s.disputePda, s.taskPda, s.escrowPda, s.claimPda, s.workerPda, voters, slashWorker);
            (0, litesvm_helpers_1.advanceClock)(svm, SLASH_WINDOW + 1);
            try {
                await program.methods
                    .applyDisputeSlash()
                    .accountsPartial(disputeSlashAccounts(s))
                    .rpc();
                chai_1.expect.fail("Should have failed with SlashWindowExpired");
            }
            catch (e) {
        ...
        it("accepts slash at exactly SLASH_WINDOW boundary", async () => {
            ...
            const s = await setupDispute("si4", test_utils_1.RESOLUTION_TYPE_REFUND, web3_js_1.LAMPORTS_PER_SOL, slashWorker, slashWorkerAgentId);
            const voters = await voteAll(s.disputePda, s.taskPda, s.claimPda, s.workerPda, [true, true, true]);
            await advanceAndResolve(s.disputePda, s.taskPda, s.escrowPda, s.claimPda, s.workerPda, voters, slashWorker);
            (0, litesvm_helpers_1.advanceClock)(svm, SLASH_WINDOW);
            await program.methods
                .applyDisputeSlash()
                .accountsPartial(disputeSlashAccounts(s))
                .rpc();
    ...
    describe("Deterministic outcome calculation (#960)", () => {
        let outcomeWorker;
        let outcomeWorkerAgentId;
        before(async () => {
            outcomeWorker = web3_js_1.Keypair.generate();
            outcomeWorkerAgentId = makeId("owk");
            airdrop([outcomeWorker]);
            await registerAgent(outcomeWorkerAgentId, outcomeWorker, test_utils_1.CAPABILITY_COMPUTE, Math.max(WORKER_STAKE, minAgentStake));
        });
        it("3 FOR / 0 AGAINST → APPROVED (worker slash succeeds)", async () => {
            const s = await setupDispute("oc1", test_utils_1.RESOLUTION_TYPE_REFUND, web3_js_1.LAMPORTS_PER_SOL, outcomeWorker, outcomeWorkerAgentId);
            const voters = await voteAll(s.disputePda, s.taskPda, s.claimPda, s.workerPda, [true, true, true]);
            await advanceAndResolve(s.disputePda, s.taskPda, s.escrowPda, s.claimPda, s.workerPda, voters, outcomeWorker);
            const d = await program.account.dispute.fetch(s.disputePda);
            (0, chai_1.expect)(d.status).to.deep.equal({ resolved: {} });
        ...
        it("0 FOR / 3 AGAINST → REJECTED (initiator slash succeeds)", async () => {
            const s = await setupDispute("oc2", test_utils_1.RESOLUTION_TYPE_REFUND, web3_js_1.LAMPORTS_PER_SOL, outcomeWorker, outcomeWorkerAgentId);
            const voters = await voteAll(s.disputePda, s.taskPda, s.claimPda, s.workerPda, [false, false, false]);
            await advanceAndResolve(s.disputePda, s.taskPda, s.escrowPda, s.claimPda, s.workerPda, voters, outcomeWorker);
            const d = await program.account.dispute.fetch(s.disputePda);
            (0, chai_1.expect)(d.status).to.deep.equal({ resolved: {} });
        ...
        it("2 FOR / 1 AGAINST → APPROVED (66% ≥ 51%)", async () => {
            const s = await setupDispute("oc3", test_utils_1.RESOLUTION_TYPE_REFUND, web3_js_1.LAMPORTS_PER_SOL, outcomeWorker, outcomeWorkerAgentId);
        ...
        it("1 FOR / 2 AGAINST → REJECTED (33% < 51%)", async () => {
            const s = await setupDispute("oc4", test_utils_1.RESOLUTION_TYPE_REFUND, web3_js_1.LAMPORTS_PER_SOL, outcomeWorker, outcomeWorkerAgentId);
    ...
    describe("Split payout correctness (#960)", () => {
        let splitWorker;
        let splitWorkerAgentId;
        before(async () => {
            splitWorker = web3_js_1.Keypair.generate();
            splitWorkerAgentId = makeId("spw");
            airdrop([splitWorker]);
            await registerAgent(splitWorkerAgentId, splitWorker, test_utils_1.CAPABILITY_COMPUTE, Math.max(WORKER_STAKE, minAgentStake));
        });
        it("split resolution gives creator the odd-lamport remainder", async () => {
            const oddReward = web3_js_1.LAMPORTS_PER_SOL + 1; // 1_000_000_001 → worker 500_000_000, creator 500_000_001
            const s = await setupDispute("sp1", test_utils_1.RESOLUTION_TYPE_SPLIT, oddReward, splitWorker, splitWorkerAgentId);
            const voters = await voteAll(s.disputePda, s.taskPda, s.claimPda, s.workerPda, [true, true, true]);
            ...
            (0, chai_1.expect)(expectedCreatorShare - expectedWorkerShare).to.equal(1);
            ...
            (0, chai_1.expect)(workerBalAfter - workerBalBefore).to.equal(expectedWorkerShare);
        });
    });
    describe("Related functionality verification", () => {
        it("should verify worker stake is tracked correctly", async () => {
            ...
            (0, chai_1.expect)(workerData.stake.toNumber()).to.be.greaterThanOrEqual(Math.max(WORKER_STAKE, minAgentStake));
        });
        it("should verify arbiters have required stake and capability", async () => {
            ...
            const hasArbiterCap = (arbiter1Data.capabilities.toNumber() & test_utils_1.CAPABILITY_ARBITER) !== 0;
            (0, chai_1.expect)(hasArbiterCap).to.be.true;
            // Arbiter should have sufficient stake
            (0, chai_1.expect)(arbiter1Data.stake.toNumber()).to.be.greaterThanOrEqual(minArbiterStake);
        });
    });
});
