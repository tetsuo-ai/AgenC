...
describe("sybil-attack", () => {
    ...
    let treasuryPubkey;
    let creator;
    let sybilAttacker; // One wallet trying to vote multiple times
    let legitimateArbiter;
    let creatorAgentPda;
    ...
    function deriveAgentPda(agentId) {
        const [pda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("agent"), agentId], program.programId);
        return pda;
    }
    function deriveVotePda(disputePda, arbiterPda) {
        const [pda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("vote"), disputePda.toBuffer(), arbiterPda.toBuffer()], program.programId);
        return pda;
    }
    ...
    before(async () => {
        ...
        sybilAttacker = web3_js_1.Keypair.generate();
        legitimateArbiter = web3_js_1.Keypair.generate();
        const thirdSigner = web3_js_1.Keypair.generate();
        const airdropAmount = 20 * web3_js_1.LAMPORTS_PER_SOL;
    ...
    describe("Sybil Attack Prevention", () => {
        ...
        before(async () => {
            ...
            taskPda = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("task"), creator.publicKey.toBuffer(), taskId], program.programId)[0];
            escrowPda = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("escrow"), taskPda.toBuffer()], program.programId)[0];
            await program.methods
                .createTask(Array.from(taskId), new bn_js_1.default(test_utils_1.CAPABILITY_COMPUTE), Buffer.from("Sybil test task".padEnd(64, "\0")), new bn_js_1.default(1 * web3_js_1.LAMPORTS_PER_SOL), 1, new bn_js_1.default(Math.floor(Date.now() / 1000) + 3600), test_utils_1.TASK_TYPE_EXCLUSIVE, null, 0, // min_reputation
            null)
                .accountsPartial({
                task: taskPda,
                escrow: escrowPda,
            ...
            await program.methods
                .registerAgent(Array.from(arbiter1Id), new bn_js_1.default(test_utils_1.CAPABILITY_ARBITER), "https://arbiter1.sybil.com", null, new bn_js_1.default(1 * web3_js_1.LAMPORTS_PER_SOL))
                .accountsPartial({
                agent: arbiter1Pda,
                protocolConfig: protocolPda,
                ...
                .rpc();
            await program.methods
                .registerAgent(Array.from(arbiter2Id), new bn_js_1.default(test_utils_1.CAPABILITY_ARBITER), "https://arbiter2.sybil.com", null, new bn_js_1.default(1 * web3_js_1.LAMPORTS_PER_SOL))
                .accountsPartial({
                agent: arbiter2Pda,
                protocolConfig: protocolPda,
                authority: sybilAttacker.publicKey,
            ...
            await program.methods
                .registerAgent(Array.from(workerAgentId), new bn_js_1.default(test_utils_1.CAPABILITY_COMPUTE), "https://worker.sybil.com", null, new bn_js_1.default(1 * web3_js_1.LAMPORTS_PER_SOL))
                .accountsPartial({
                ...
                .signers([sybilAttacker])
                .rpc();
            workerClaimPda = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("claim"), taskPda.toBuffer(), workerAgentPda.toBuffer()], program.programId)[0];
            await program.methods
                .claimTask()
                .accountsPartial({
                task: taskPda,
            ...
            disputePda = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("dispute"), disputeId], program.programId)[0];
            await program.methods
                .initiateDispute(Array.from(disputeId), Array.from(taskId), Array.from(Buffer.from("evidence".padEnd(32, "\0"))), test_utils_1.RESOLUTION_TYPE_REFUND, VALID_EVIDENCE)
                .accountsPartial({
                dispute: disputePda,
                task: taskPda,
                agent: workerAgentPda,
        ...
        it("First vote from authority succeeds", async () => {
            ...
            try {
                await program.methods
                    ...
                    .signers([sybilAttacker])
                    .rpc();
                chai_1.expect.fail("Participant authority should not be allowed to vote");
            }
            catch (e) {
                (0, chai_1.expect)(e.message).to.include("ArbiterIsDisputeParticipant");
            }
        });
        it("Second vote from same authority (via different agent) is prevented", async () => {
            ...
            try {
                await program.methods
                    ...
                    .signers([sybilAttacker])
                    .rpc();
                chai_1.expect.fail("Should have failed - authority already voted");
            }
            catch (e) {
        ...
        it("Different authority can still vote", async () => {
            ...
            await program.methods
                .registerAgent(Array.from(legitArbId), new bn_js_1.default(test_utils_1.CAPABILITY_ARBITER), "https://legit-arbiter.example.com", null, new bn_js_1.default(1 * web3_js_1.LAMPORTS_PER_SOL))
                .accountsPartial({
                agent: legitArbPda,
                protocolConfig: protocolPda,
            ...
            (0, chai_1.expect)(dispute.totalVoters).to.equal(1);
            (0, chai_1.expect)(dispute.votesFor.toNumber()).to.equal(0);
            (0, chai_1.expect)(dispute.votesAgainst.toNumber()).to.be.greaterThan(0);
        });
        it("AuthorityDisputeVote account contains correct data", async () => {
            ...
            (0, chai_1.expect)(missingSybilVote).to.be.true;
            ...
            (0, chai_1.expect)(legitVote.dispute.toString()).to.equal(disputePda.toString());
            (0, chai_1.expect)(legitVote.authority.toString()).to.equal(legitimateArbiter.publicKey.toString());
        });
    });
    describe("Multiple Disputes - Same Authority Can Vote on Different Disputes", () => {
        it("Same authority can vote once per dispute", async () => {
            ...
            const multiArbiter = web3_js_1.Keypair.generate();
            await provider.connection.confirmTransaction(await provider.connection.requestAirdrop(multiArbiter.publicKey, 5 * web3_js_1.LAMPORTS_PER_SOL), "confirmed");
            await program.methods
                .registerAgent(Array.from(arb3Id), new bn_js_1.default(test_utils_1.CAPABILITY_ARBITER), "https://multi-arbiter.example.com", null, new bn_js_1.default(1 * web3_js_1.LAMPORTS_PER_SOL))
                .accountsPartial({
                agent: arb3Pda,
                protocolConfig: protocolPda,
            ...
            await program.methods
                .registerAgent(Array.from(multiWorkerId), new bn_js_1.default(test_utils_1.CAPABILITY_COMPUTE), "https://multi-worker.example.com", null, new bn_js_1.default(1 * web3_js_1.LAMPORTS_PER_SOL))
                .accountsPartial({
            ...
            for (const taskId of [task1Id, task2Id]) {
                const taskPda = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("task"), creator.publicKey.toBuffer(), taskId], program.programId)[0];
                await program.methods
                    .createTask(Array.from(taskId), new bn_js_1.default(test_utils_1.CAPABILITY_COMPUTE), Buffer.from("Multi dispute task".padEnd(64, "\0")), new bn_js_1.default(1 * web3_js_1.LAMPORTS_PER_SOL), 1, new bn_js_1.default(Math.floor(Date.now() / 1000) + 3600), test_utils_1.TASK_TYPE_EXCLUSIVE, null, 0, // min_reputation
                null)
                    .accountsPartial({
                    task: taskPda,
                    escrow: web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("escrow"), taskPda.toBuffer()], program.programId)[0],
                    protocolConfig: protocolPda,
                    creatorAgent: creatorAgentPda,
                    authority: creator.publicKey,
                    creator: creator.publicKey,
                    ...
                    .rpc();
                // Claim the task
                const claimPda = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("claim"), taskPda.toBuffer(), multiWorkerPda.toBuffer()], program.programId)[0];
                await program.methods
                    .claimTask()
                    .accountsPartial({
                    task: taskPda,
            ...
            const task1Pda = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("task"), creator.publicKey.toBuffer(), task1Id], program.programId)[0];
            const task2Pda = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("task"), creator.publicKey.toBuffer(), task2Id], program.programId)[0];
            const claim1Pda = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("claim"), task1Pda.toBuffer(), multiWorkerPda.toBuffer()], program.programId)[0];
            const claim2Pda = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("claim"), task2Pda.toBuffer(), multiWorkerPda.toBuffer()], program.programId)[0];
            const dispute1Pda = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("dispute"), dispute1Id], program.programId)[0];
            const dispute2Pda = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from("dispute"), dispute2Id], program.programId)[0];
            // Initiate both disputes
            await program.methods
                .initiateDispute(Array.from(dispute1Id), Array.from(task1Id), Array.from(Buffer.from("evidence1".padEnd(32, "\0"))), test_utils_1.RESOLUTION_TYPE_REFUND, VALID_EVIDENCE)
                .accountsPartial({
                dispute: dispute1Pda,
                task: task1Pda,
                agent: multiWorkerPda,
                ...
                .rpc();
            await program.methods
                .initiateDispute(Array.from(dispute2Id), Array.from(task2Id), Array.from(Buffer.from("evidence2".padEnd(32, "\0"))), test_utils_1.RESOLUTION_TYPE_REFUND, VALID_EVIDENCE)
                .accountsPartial({
                dispute: dispute2Pda,
                task: task2Pda,
                agent: multiWorkerPda,
